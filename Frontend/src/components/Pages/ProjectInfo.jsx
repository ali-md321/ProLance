  ...
  return (
    <div style={{ fontFamily:"'DM Sans',sans-serif" }}>
      <style>{`
        ...
        .proposals-btn{background:linear-gradient(135deg,rgba(99,102,241,0.2),rgba(168,85,247,0.15));border:1px solid rgba(99,102,241,0.4);color:#a5b4fc;position:relative;overflow:hidden;}
        .proposals-btn::before{content:'';position:absolute;inset:0;background:linear-gradient(135deg,#6366f1,#a855f7);opacity:0;transition:opacity .2s;}
        .proposals-btn:hover::before{opacity:1;}
        .proposals-btn:hover{color:#fff;box-shadow:0 0 24px rgba(99,102,241,0.4);}
        .proposals-btn span{position:relative;z-index:1;}
      `}</style>
      ...
      <div className="max-w-5xl mx-auto space-y-5">
          ...
          <div className="relative px-6 md:px-8 pt-7 pb-6"
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              ...
              <div className="px-3 py-1 rounded-full text-xs font-semibold"
                style={{ border:"1px solid rgba(99,102,241,0.2)", color:pay.color, background:"rgba(13,12,28,0.6)" }}>
            ...
            {isClient && (
              <div className="flex flex-wrap gap-3 mt-4">
                ...
                {/* ── SEE PROPOSALS BUTTON ── */}
                <motion.button whileHover={{ scale:1.03 }} whileTap={{ scale:.97 }}
                  onClick={() => navigate(`/projects/${project._id}/proposals`)}
                  className="proposals-btn flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all">
                  <span className="flex items-center gap-2">
                    <FileText size={14}/>
                    See Proposals
                    {project.totalProposals > 0 && (
        ...
        <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ duration:.5, delay:.1 }}
          ...
          <StatCard icon={Clock}      label="Deadline"   value={project.deadline ? new Date(project.deadline).toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"}) : "Not set"}/>
